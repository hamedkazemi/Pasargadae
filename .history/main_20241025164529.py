...
class FastBlur:
    ...
    def blur(image, radius):
        if radius < 1:
            return image
        if image is None:
            raise CriticalError("Cannot blur null image")
        ...
        try:
            ...
            raise CriticalError(f"Error during blur operation: {str(e)}")
...
class TransparentWindow(QMainWindow):
    ...
    def updateBackground(self):
        try:
            ...
            if not screen:
                raise CriticalError("Failed to get primary screen")
            ...
            if screen_pixmap.isNull():
                raise CriticalError("Failed to capture screen")
            ...
            raise CriticalError(f"Failed to update background: {str(e)}")
    ...
    def paintEvent(self, event):
        try:
            ...
            raise CriticalError(f"Paint event failed: {str(e)}")

    def moveEvent(self, event):
        try:
            super().moveEvent(event)
            # Update background when window moves
            self.updateBackground()
        except Exception as e:
            raise CriticalError(f"Move event failed: {str(e)}")
    ...
    def showEvent(self, event):
        try:
            ...
            raise CriticalError(f"Show event failed: {str(e)}")
