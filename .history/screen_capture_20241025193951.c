...
struct ScreenBuffer* init_screen_capture() {
    ...
    screen->loop = pw_main_loop_new(NULL);
    if (!screen->loop) {
        free(screen);
        return NULL;
    }

    screen->context = pw_context_new(pw_main_loop_get_loop(screen->loop), NULL, 0);
    if (!screen->context) {
        pw_main_loop_destroy(screen->loop);
        free(screen);
        return NULL;
    }
    ...
    screen->stream = pw_stream_new_simple(
        pw_main_loop_get_loop(screen->loop),
        "screen-capture",
        props,
        &stream_events,
        screen);
...
void capture_loop(struct ScreenBuffer* screen) {
    ...
    while (running) {
        ...
        pw_main_loop_iterate(screen->loop, 0);
        render_frame(screen);
